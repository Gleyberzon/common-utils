...
public class ZimLoginPage extends BasePage {
    ...
    public void logIn(String user, String password)
    {
        ...
        try {
            txt_user = WaitWrapper.waitForVisibilityOfElementLocated(driver, By.name("loginfmt"), timeout);
            logger.info(String.format("Enter username: '%s'", user));
            txt_user.sendKeys(user);
        }
        catch(Exception e){
            throw new Error("Username field could not be found on login page");
        }
        ...
        try {
            txt_password = WaitWrapper.waitForVisibilityOfElementLocated(driver, By.name("passwd"), timeout);
            logger.info(String.format("Enter password: '%s'", password));
            txt_password.sendKeys(password);
        }
        catch(Exception e2){
            throw new Error("Password field could not be found on login page");
        }
