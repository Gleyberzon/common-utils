...
public class Table extends TesnetWebElement {
    ...
    protected final LinkedHashMap<Integer, String> headersDetails;
    ...
    public WebElement getHeaderRowElement()
    {
        ...
        {
            logger.info("No header displays in the table");
            return null;
        }
    }
     ...
     * @since 07.03.2022
     */
    private LinkedHashMap<Integer, String> retrieveHeadersDetails()
    {
    ...
    public List<WebElement> waitForRowsElements()
    {
        ...
        try {
            return new LinkedList<>( WaitWrapper.waitForVisibilityOfAllElementsLocatedBy(driver, localElement, By.xpath(".//tbody/tr"), 30));
        }
        catch (Exception e)
        {
            return new ArrayList<>();
    ...
    public <T extends TableRow> List<T> getRows(Class<T> classType) {
        ...
        LinkedList<T> tableRows = new LinkedList<>();
        for (WebElement rowElement: rowsElements){
            TableRow row = ObjectsUtils.newInstance(classType, driver, rowElement, this);
            tableRows.add((T)row);
        }
        return tableRows;
    }
    ...
    private List<WebElement> getHeaderCells(){
        return getHeaderRow().findElements(By.tagName("th"));
    }
